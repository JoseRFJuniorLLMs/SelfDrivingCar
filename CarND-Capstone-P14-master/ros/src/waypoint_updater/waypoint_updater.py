...
class WaypointUpdater(object):
    ...
    def get_closest_waypoints_idx(self):
        ...
        # finding closest waypoint index using KD tree
        closest_idx = self.waypoint_tree.query([x,y],1)[1]

        # to check if closest waypoint is ahead or behind car's current pos
        closest_coord = self.waypoints_2d[closest_idx]
        previous_coord = self.waypoints_2d[closest_idx - 1]

        # making vector of points
        ...
        if val > 0:
            # closest waypoint choosen is behind the car
